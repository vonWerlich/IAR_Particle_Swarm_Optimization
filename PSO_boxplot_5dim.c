...
double run_pso(Problem prob, Variant variant) {
	...
	double c1 = 3.05, c2 = 1.05;  //c1 -> pessoal // c2 -> social
...
int main() {
    ...
    srand(time(NULL));
    FILE *f = fopen("boxplot_1.txt", "w");
    if (!f) {
        perror("Erro ao abrir o arquivo de saída");
        return 1;
    }
	
	Problem problema = GRIEWANK;         // GRIEWANK ou ACKLEY
    Variant variante = STANDARD;        // STANDARD, INERTIA ou CONSTRICTION
    int i;
	for (i=0; i<30; i++) {
