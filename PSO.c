...
void run_pso(Problem prob, Variant variant) {
    ...
    double w = 0.7; //peso de inércia
	double c1 = 2.025, c2 = 2.025;  //c1 -> pessoal // c2 -> social
    ...
    fprintf(f, "iter,current_best,gbest_value\n");
    ...
    for (int t = 0; t < MAX_ITER; t++) {
		...
		fprintf(f, "%d,%.10f,%.10f\n", t, best_current, gbest_val);// salvar ambos no CSV
    }
    fclose(f);
    printf("Melhor global: %.10f\n", gbest_val);
}

int main() {
    Problem problema = GRIEWANK;         // ACKLEY ou GRIEWANK
    Variant variante = CONSTRICTION;        // STANDARD, INERTIA ou CONSTRICTION

    run_pso(problema, variante);
    return 0;
}
/*
